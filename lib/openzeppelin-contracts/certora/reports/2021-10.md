Formal Verification Report for
...
submitted as a pull request against OpenZeppelin’s public git repository.
...
Two systems that use the same timelock can attack each other
...
Voting period is 1 block less than expected
...
Fixed in a more recent commit. A proposal is considered Pending
...
OpenZeppelin Contracts documentation for a high-level overview of the Governance

module, and the Governance API for a detailed description.
...
were based on the output of the OpenZeppelin Contracts Wizard. We then wrote general-
...
Assumptions and simplifications made during
verification
...
proposalSnapshot(proposalId) > 0 ⇒  proposalSnapshot(proposalId) ≤ proposalDeadline(propos
...

{
    ...
    <transaction>
{
    e.block.number < proposalDeadline(proposalId) ⇒ (¬isExecuted(proposalId)
}
...
allFunctionsRevertIfExecuted  and  allFunctionsRevertIfCanceled
    ...
    <transaction on proposalId>
{
    ...
    ((¬hasVotedBefore ∧ hasVotedAfter) ⇒ againstBefore ≤ againstAfter ∨ forBefore ≤ forAft
}
...
is contained in the  _castVote  helper method. ↩ ↩
