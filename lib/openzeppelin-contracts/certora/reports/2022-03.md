Formal Verification of OpenZeppelin
(March - April 2022)
Summary
...
latest commit that was reviewed and ran through the Certora Prover was 4088540a .
...
The formal specifications have been submitted as a pull request against OpenZeppelin’s
...
|     | Anyone can call  |           |  for a   | . An attacker can call |
| --- | ---------------- | --------- | -------- | ---------------------- |
|     |                  | flashLoan | receiver |                        |
Description: flashLoan  repeatedly on a receiver and drain its funds as the
...
Description:
. If this is indeed the case, they should mention it
type(uint224).max
...
Response: . Both are limited, by design, to uint224. We will improve
ERC20Votes
...
Response: The check should should indeed be removed from  Votes.sol  to save
gas
Severity: Informational
...
Response: chain design. Even if someone was to use block.timestamp based
...
∧ wrapperUserBalanceBefore == wrapperUserBalanceAfter + amount))
∧ (account ≠ msg.sender
⇒ (wrapperSenderBalanceBefore == wrapperSenderBalanceAfter + amount
...
index1 > index2 ⇒ ckptFromBlock(account, index1) > ckptFromBlock(account, index2)
...
fromBlock_ = num_ckpts_ == 0 ? 0 : ckptFromBlock(account, num_ckpts_ - 1)
}
<arbitrary function f>
{
_num_ckpts = numCheckpoints(account);
_fromBlock = _num_ckpts == 0 ? 0 : ckptFromBlock(account, _num_ckpts - 1)
fromBlock_ == _fromBlock ⇒ num_ckpts_ == _num_ckpts v _num_ckpts == 1
}
( ) transfer_safe()
...
safeTransferFrom(from, to, id, amount, data) ~ safeTransferFrom(from, to, id, amount1,
...
amount == amount1 + amount2 ∧
_mint(to, id, amount, data) ~ _mint(to, id, amount1, data); _mint(to, id, amount2, data
Equivalent with respect to the balanceOf(from, id)
...
amount == amount1 + amount2 ∧
_burn(from, id, amount) ~ _burn(from, id, amount1); _burn(from, id, amount2)
Equivalent with respect to the balanceOf(from, id)
( ) burnRevertCases
...
block.timestamp for the same msg.sender, etc. More detailed: link ↩
