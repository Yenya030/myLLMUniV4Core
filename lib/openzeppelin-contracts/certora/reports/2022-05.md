Formal Verification of OpenZeppelin (May -
June 2022)
Summary
...
commit that was reviewed and run through the Certora Prover was commit 109778c .
...
specifications of Open Zeppelin. All the rules are publically available in a public github.
List of Main Issues Discovered
Severity: High Medium Low

|     | Calling  updateQuorumNumerator() |     |  can change the output of |     |
| --- | -------------------------------- | --- | ------------------------- | --- |
Issue: quorumReached()  for previous proposals, leading to unexpected
outcomes.
| Rules   | quorumReachedEffect | ,  proposalNotCreatedEffects |     | ,   |
| ------- | ------------------- | ---------------------------- | --- | --- |
| Broken: |                     | ,                            |     |     |
...
Description:
extending their deadlines. Breaks rules  ,
quorumReachedEffect
|     | proposalNotCreatedEffects |     | , and  proposalInOneState | .   |
| --- | ------------------------- | --- | ------------------------- | --- |
...
| --- | --------------------------- | --- | -------------------------- | --- |
Response: requirements do not affect past proposals. Additionally, we are looking
...
Issue: consider all current and future proposals to have reached
quorum.
Rules quorumReachedEffect , proposalNotCreatedEffects ,
Broken: proposalInOneState
...
Description: as interesting because there will be no tokens to vote with. A more
...
Response: balances, as in this case no one will be able to vote for a proposal and
...
Response: consequence is that if the proposal is executed directly in the
...
ERC1155 establishes base level support EIP1155, a standard interface for contracts that
...
the previous report for earlier contract properties verified.
...
| • Dummy functions created to verify  |            |  and          |     |  modifiers |
| ------------------------------------ | ---------- | ------------- | --- | ---------- |
|                                      | whenPaused | whenNotPaused |     |            |
Properties
...
must not change.
( ) rule
transferMethodsRevertWhenPaused
When a contract is in a paused state, transfer methods must revert.
| ( ) rule  pauseMethodPausesContract |     |     |     |     |
| ----------------------------------- | --- | --- | --- | --- |
When a contract is in an unpaused state, calling   must transition to a paused
pause
state.
...
| -------------------------------------------------- | --- | --- | --- | --- |
When a contract is in an unpaused state, functions with the   modifier
whenPaused
must revert.
...
Assumptions and Simplifications
| • The  |  method was wrapped in the  |     |  method because  |     |
| ------ | --------------------------- | --- | ---------------- | --- |
exists exists_wrapper exists

is a keyword in CVL.
| • The public functions  |      | ,         | ,  , and  |           |  were implemented |
| ----------------------- | ---- | --------- | --------- | --------- | ----------------- |
|                         | burn | burnBatch | mint      | mintBatch |                   |
...
| ( ) Bug1: mutate  |           |  function in  |                               |     |     |
| ----------------- | --------- | ------------- | ----------------------------- | --- | --- |
|                   | _castVote |               | GovernorPreventLateQuorum.sol |     |     |
...
- require(account == _msgSender() || isApprovedForAll(account, _msgSender()),"ERC1155: cal
+ // require(account == _msgSender() || isApprovedForAll(account, _msgSender()),"ERC1155:
...
multipleTokenBurnBurnBatchEquivalence [Tool Output]
